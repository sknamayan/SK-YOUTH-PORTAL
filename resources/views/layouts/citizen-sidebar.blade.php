...
<aside class="citizen-sidebar fixed md:sticky md:self-start inset-y-0 md:bottom-auto left-0 w-[86%] max-w-[300px] sm:w-[80%] md:w-64 lg:w-72 xl:w-80 md:max-w-none md:top-16 md:h-[calc(100vh_-_4rem)] border-r border-white/10 bg-[#1e40af] z-30 transition-transform duration-300 transform flex flex-col justify-between shrink-0 shadow-sm md:shadow-none overflow-x-hidden">
    ...
    <div class="flex-1 p-4 sm:p-6 space-y-5 sm:space-y-6 overflow-y-auto overflow-x-hidden">
        ...
        <nav class="space-y-1.5">
            ...
            <a href="{{ route('profile.my-requests') }}"
               class="flex items-center justify-between w-full px-4 py-2.5 rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider transition {{ request()->routeIs('profile.my-requests') ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            ...
            <a href="{{ route('forms.sports.create') }}"
               class="flex items-center justify-between w-full px-4 py-2.5 rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider transition {{ request()->url() === route('forms.sports.create') || request()->query('form') === 'sports' ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
                <div class="flex items-center gap-3 min-w-0">
            ...
            <a href="{{ route('profile.profiling.create') }}"
               class="flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider transition {{ request()->routeIs('profile.profiling.create') ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            ...
            <a href="{{ route('profile.edit') }}"
               class="flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider transition {{ request()->routeIs('profile.edit') ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            ...
            <!-- View Website Link -->
            <a href="/"
               class="flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider transition sidebar-link-inactive">
    ...
    @auth
    <div class="px-4 sm:px-6 py-6 sm:py-8 sidebar-footer border-t flex items-center justify-between shrink-0 relative">
        <div class="flex items-center gap-3 min-w-0">
