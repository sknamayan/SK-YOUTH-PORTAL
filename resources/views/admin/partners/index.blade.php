    ...
    <div class="flex-1 flex flex-col min-w-0 min-h-0 md:min-h-[calc(100dvh-4rem)]">
        ...
        <header class="sticky top-0 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 h-14 px-3 flex items-center justify-between md:hidden shrink-0 supports-[padding:max(0px)]:pt-[env(safe-area-inset-top)]">
            ...
            <div class="flex items-center gap-2 min-w-0">
                ...
                <span class="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-100 font-display truncate">Sponsors</span>
            </div>
        ...
        @if(Auth::user()->isAdmin())
            <div class="fixed bottom-0 inset-x-0 z-20 md:hidden pointer-events-none px-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
                <button
                    ...
                    aria-label="Add new sponsor partner"
                >
        ...
        Alpine.data('partnersAdmin', (config = {}) => ({
            ...
            init() {
                ...
                this.$watch('mobileSidebar', (open) => {
                    if (open) {
                        this.openAddSponsorModal = false;
                        this.openEditSponsorModal = false;
                    }
                });
            },
