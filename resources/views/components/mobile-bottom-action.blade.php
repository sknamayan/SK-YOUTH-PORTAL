...
{{--
    ...
    This component correctly forwards layout and Alpine display directives 
    (like x-show, x-cloak, class) to the outer fixed wrapper, while keeping 
    form/action-specific attributes (like disabled, form, onclick, @click) 
    on the inner interactive element to prevent click-blocking layout bugs.
--}}

<div 
    {{ $attributes->except(['form', 'type', 'href', 'disabled', 'onclick', 'x-bind:disabled', ':disabled', 'wire:click', 'wire:target', 'wire:loading.attr'])->merge([
...
>
    <div class="px-4 py-3">
        @if($href)
            <a
                href="{{ $href }}"
                {{ $attributes->only(['disabled', 'onclick', '@click', 'x-on:click', 'x-bind:disabled', ':disabled', 'wire:click', 'wire:target', 'wire:loading.attr'])->merge([
