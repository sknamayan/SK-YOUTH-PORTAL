...
<div class="card max-w-2xl mx-auto overflow-hidden border border-slate-800 dark:border-slate-750 bg-slate-900 shadow-2xl" x-data="{ submitting: false }">
    ...
    <form method="POST" action="{{ $action }}" class="request-form space-y-5" @if($enctype) enctype="{{ $enctype }}" @endif x-on:submit="submitting = true">
        @csrf
