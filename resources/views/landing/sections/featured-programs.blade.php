    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        @foreach($categories as $key => $cat)
            <a href="{{ route('projects.committee', ['project_slug' => 'sk-namayan-youth-services', 'committee_slug' => $key]) }}"
               class="min-h-[140px] py-6 px-3 bg-white border border-slate-100 hover:border-[#1e40af] text-slate-700 hover:text-[#1e40af] hover:-translate-y-1 hover:shadow-md transition-all duration-300 rounded-2xl flex flex-col justify-center items-center text-center group relative overflow-hidden active:scale-95">
